...
var Select = React.createClass({
   ...
   */
	...
	selectValue: function selectValue(value, focusAfterUpdate) {
		if (!this.props.multi) {
			this.setValue(value, focusAfterUpdate);
		} else if (value) {
			this.addValue(value, focusAfterUpdate);
		}
		this._unbindCloseMenuIfClickedOutside();
	},

	addValue: function addValue(value, focusAfterUpdate) {
		this.setValue(this.state.values.concat(value), focusAfterUpdate);
	},
	...
	modifyValue: function modifyValue(valueToModify, newValue) {
	...
	handleInputBlur: function handleInputBlur(event) {
		...
		this._blurTimeout = setTimeout(function () {
			...
			if (_this7.state.inputValue) {
				if (! _this7.props.allowCreate && ! _this7.state.focusedOption) _this7.focusNextOption();
				_this7.selectFocusedOption(false);
			}
	...
	loadAsyncOptions: function loadAsyncOptions(input, state, callback) {
		...
		var optionsResponseHandler = function optionsResponseHandler(err, data) {
			...
			if ((_this9.props.autoload && thisRequestId > 0) && thisRequestId !== _this9._currentRequestId) {
				return;
			}
	...
	buildMenu: function buildMenu() {
		...
		var ops = Object.keys(options).map(function (key) {
			var op = options[key];
			var val = op[this.props.valueKey];
			var isSelected = this.state.value && this.state.value.split(',').includes(val);
			var isFocused = focusedValue === val;
